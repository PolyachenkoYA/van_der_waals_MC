...
#define gen_mode_ID_solidUvacuum 2
#define N_gen_modes 3
...
//	template <typename T> char sgn(T val) { return (T(0) < val) - (val < T(0));	}
	...
	void md_3D(double *X, const double *L);
	...
	void get_random_3D_point(double *X, const double *L);
...
//						  long *Nt, long *OP_arr_len, int verbose);
	...
	bool X_fits_in(const double *X_new, const double *X_present, int N_atoms_present, const double *R, double r2_min=1);
